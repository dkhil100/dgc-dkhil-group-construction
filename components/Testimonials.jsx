...
const row1 = [
  ...
  },
  {
    ...
    avatar: "J",
    color: "bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border-blue-500/30",
      ...
      >
        ...
        <div className="flex gap-6 w-max animate-marquee hover:[animation-play-state:paused] py-3">
          {[...row1, ...row1].map((t, index) => (
            <div
              key={index}
              className="w-[320px] md:w-[380px] p-6 rounded-2xl bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 backdrop-blur-md flex flex-col justify-between shrink-0 shadow-lg transition-colors duration-300"
            >
              <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mb-6 transition-colors duration-300">
                "{t.text}"
              </p>
              <div className="flex items-center gap-3 pt-4 border-t border-slate-200 dark:border-slate-800/60 transition-colors duration-300">
                <div
                  className={`w-9 h-9 rounded-full border flex items-center justify-center font-bold text-xs transition-colors duration-300 ${t.color}`}
                >
                  {t.avatar}
                </div>
                <div>
                  <h4 className="text-slate-900 dark:text-white text-sm font-semibold transition-colors duration-300">
                    {t.name}
                  </h4>
                  <p className="text-slate-500 dark:text-slate-400 text-xs transition-colors duration-300">
                    {t.role}
                  </p>
                </div>
              </div>
